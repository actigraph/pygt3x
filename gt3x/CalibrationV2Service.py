...
class CalibrationV2Service:
    ...
    def set_calibration(self, calibration: dict, sample_rate: int):
        ...
        self.sensitivity_matrix = np.array([[s11, s21, s31], [s12, s22, s32], [s13, s23, s33]])

    def calibrate_samples(self, samples):
        for sample in samples:
            calibrated_sample = np.matmul(self.sensitivity_matrix, (
                        np.array([[sample.x, sample.y, sample.z]]) - self.offset_vector).transpose()).transpose()
            yield gt3x.AccelerationSample(sample.timestamp, calibrated_sample[0][0], calibrated_sample[0][1],
